...
var getEnv = function() {
	var P = {
		...
		VariableStatement: function (t, env) { getEnv(t.declarations, env); },
		VariableDeclaration: function (t, env) { env.local.push(t.name); },
		PointerDeclaration: function (t, env) { env.local.push(t.name); },
		...
		IfStatement: function (t, env) {
			...
			var environments = {};
			environments = { var_list: env.var_list, local: [], make_act: [0], make_msg: [0] };
			getEnv(t.if_statement, environments);
			tmp_local["if"] = environments.local;
			tmp_act["if"] = environments.make_act;
			tmp_msg["if"] = environments.make_msg;
			if (t.else_statement) {
				environments = { var_list: env.var_list, local: [], make_act: [0], make_msg: [0] };
				getEnv(t.else_statement, environments);
			...
			env.make_msg.push(tmp_msg, 0);
		},
		WhileStatement: function (t, env) {
			var tmp_local = {}, tmp_act = {}, tmp_msg = {};
			var environments = {};
			environments = { var_list: env.var_list, local: [], make_act: [0], make_msg: [0] };
			getEnv(t.statement, environments);
		...
		ForStatement: function (t, env) {
			...
			if (!t.initializer) {
				environments = { var_list: env.var_list, local: [], make_act: [0], make_msg: [0] };
				getEnv(t.initializer, environments);
		...
		SwitchStatement: function (t, env) {
			...
			t.clauses.forEach( function (t0, i) {
				environments = { var_list: env.var_list, local: [], make_act: [0], make_msg: [0] };
				getEnv(t0.statements, environments);
	...
	function (err, input) {
		...
		actor_names.forEach( function (t0) {
			...
			methods.forEach(function (t1) {
				var actor_method_var_list = actor_var_list[t1] = {this: this_list, params: {}, local: {}};
